...
export function handlegetUserBalance(uid: string , data : string){
   try {
    ...
    if(!userBalance){
     ...
     publishMessage(`channel_${uid}` ,`User Balance ${userBalance.balance}`);
      return;
   } catch (error : any) {
...
try {
      ...
      if(!stockBalances){
       ...
       publishMessage(`channel_${uid}` ,`your stock  Balance is  ${JSON.stringify(stockBalances)}`);
       return;
    } catch (error : any) {
    publishMessage(`channel_${uid}` , {error: error.message, status :"Error in fetching the user stock balance"});
}
}
