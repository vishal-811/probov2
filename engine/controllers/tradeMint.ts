...
export function handleTradeMint(uid: string, data: any) {
  try {
    ...
    if (!userId || !stockSymbol || quantity <= 0 || price <= 0) {
      throw new Error(
        "Please provide userId, stockSymbol, quantity, and price."
      );
    }
    ...
    publishMessage(
      `channel_${uid}`,
      `Minted ${quantity} 'yes' and 'no' tokens for user ${userId}, remaining balance is ${INR_BALANCES[userId].balance}`
    );
    return;
  } catch (error: any) {
